...
def analyze_set_file_with_onnx(filepath, model_path='model/model_onnx/va_regressor.onnx', compute_phi=False, phi_method='mock'):
...
def main():
    ...
    set_file = sys.argv[1]
    model_path = sys.argv[2] if len(sys.argv) > 2 and not sys.argv[2].startswith('--') else 'model/model_onnx/va_regressor.onnx'
