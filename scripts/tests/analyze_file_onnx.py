...
def parse_args():
    ...
    ap.add_argument("--output_interval", type=float, default=0.1,
                   help="Output interval in seconds (delay between data points)")
...
def main():
    ...
    try:
        ...
        onnx = ONNXRunner("model/model_onnx/va_regressor.onnx", use_gpu=True)
        ...
        for start in range(0, data.shape[1] - win, step):
            ...
            # Add configurable delay between outputs for real-time simulation
            if args.output_interval > 0:
                time.sleep(args.output_interval)
