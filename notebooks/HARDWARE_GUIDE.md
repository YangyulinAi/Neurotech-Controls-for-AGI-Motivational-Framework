...
### 2. X.on EEG System
- **Channels**: 8 (F3, F4, P3, P4, T7, T8, O1, O2)
- **Sampling Rate**: 500 Hz (auto-resampled to 256 Hz)
- **FAA Channels**: F3, F4
- **Reference**: A1, A2 (ear references)
- **Best For**: Research applications, high-quality emotion recognition
- **Features**: LSL compatible, automatic resampling, real-time processing
...
```bash
# During training
python scripts/train/train_labeled.py --device_name Muse2

# During real-time inference  
python scripts/main.py --device Muse2
```
...
python scripts/main.py --input_source lsl
```
...
Add your device to `configs/device_mapping.json`:
...
adapter = DeviceAdapter()
adapter.set_device('MyCustomDevice')
...
3. **Connect to system**:
```bash
python scripts/lsl_receiver.py --device X.on
```
...
python scripts/tools/inference_benchmark.py --model model/model_onnx/va_regressor.onnx --iterations 1
