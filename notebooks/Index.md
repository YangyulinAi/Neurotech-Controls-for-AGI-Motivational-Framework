# Neural Axis BCI - Documentation Hub

Welcome to the Neural Axis BCI documentation center. This hub provides organized access to all system documentation and guides.

## 🏠 Quick Navigation

> 🗺️ **Navigation Guide**: [NAVIGATION.md](NAVIGATION.md)

### 📚 **Core Documentation**
- **[📋 Main README](README.md)** - Project overview, installation, and quick start
- **[📑 Technical Documentation](DOCUMENTATION.md)** - Detailed technical reference

### 🔧 **Hardware & Setup**
- **[🧠 Hardware Guide](HARDWARE_GUIDE.md)** - Complete EEG device setup
- **[📡 X.on Setup Guide](X_on_Setup_Guide.md)** - X.on device specific configuration
- **[⚡ Quick Start Tutorial](quick_start.ipynb)** - Interactive system walkthrough

### 🧬 **Advanced Features**
- **[🌀 IIT Φ Integration](README_IIT_PHI.md)** - Consciousness measurement system

---

## 📖 Documentation Structure

### **Getting Started**
| Document | Description | Audience |
|----------|-------------|----------|
| [README.md](README.md) | Main project overview and quick start | All users |
| [Quick Start Tutorial](quick_start.ipynb) | Interactive walkthrough | New users |
| [Hardware Guide](HARDWARE_GUIDE.md) | Device setup instructions | Hardware users |

### **Technical Reference**
| Document | Description | Audience |
|----------|-------------|----------|
| [Technical Documentation](DOCUMENTATION.md) | Technical documentation hub | Developers |
| [IIT Φ Integration](README_IIT_PHI.md) | Consciousness measurement | Researchers |

### **Specialized Guides**
| Document | Description | Audience |
|----------|-------------|----------|
| [X.on Setup Guide](X_on_Setup_Guide.md) | X.on device configuration | X.on users |

---

## 🎯 **Quick Access by Use Case**

### **I'm New to Neural Axis BCI**
1. Start with [README.md](README.md) for overview
2. Follow [Quick Start Tutorial](quick_start.ipynb) for hands-on experience
3. Check [Hardware Guide](HARDWARE_GUIDE.md) for device setup

### **I Want to Set Up Hardware**
1. [Hardware Guide](HARDWARE_GUIDE.md) - General device setup
2. [X.on Setup Guide](X_on_Setup_Guide.md) - If using X.on device

### **I'm a Developer/Researcher**
1. [Technical Documentation](DOCUMENTATION.md) - Technical details
2. [IIT Φ Integration](README_IIT_PHI.md) - Consciousness measurement

### **I Need Complete System Understanding**
1. [README.md](README.md) - Project overview
2. [Technical Documentation](DOCUMENTATION.md) - Technical reference
3. [Hardware Guide](HARDWARE_GUIDE.md) - Device integration

---

## 🔗 **Cross-Reference Links**

### **System Architecture**
- [Frontend Architecture](README.md#system-architecture) → Main README
- [Backend Services](README.md#system-architecture) → Main README
- [Python Analysis Engine](README.md#system-architecture) → Main README

### **Hardware Integration**
- [Device Configuration](HARDWARE_GUIDE.md#device-configuration) → Hardware Guide
- [Channel Mapping](HARDWARE_GUIDE.md#channel-mapping) → Hardware Guide
- [X.on Specific Setup](X_on_Setup_Guide.md#x-on-setup) → X.on Setup Guide

### **Advanced Features**
- [IIT Φ Calculation](README_IIT_PHI.md#phi-calculation) → IIT Φ Integration
- [Model Training](README.md#usage) → Main README

---

## 📊 **Documentation Status**

| Document | Status | Last Updated | Version |
|----------|--------|--------------|---------|
| [README.md](README.md) | ✅ Complete | 2025-01 | 2.0 |
| [Hardware Guide](HARDWARE_GUIDE.md) | ✅ Complete | 2025-01 | 2.0 |
| [X.on Setup Guide](X_on_Setup_Guide.md) | ✅ Complete | 2025-01 | 2.0 |
| [IIT Φ Integration](README_IIT_PHI.md) | ✅ Complete | 2025-01 | 2.0 |
| [Technical Documentation](DOCUMENTATION.md) | ✅ Complete | 2025-01 | 2.0 |
| [Quick Start Tutorial](quick_start.ipynb) | ✅ Complete | 2025-01 | 2.0 |

---

## 🆘 **Need Help?**

### **Common Issues**
- **Installation Problems** → [README.md#installation](README.md#installation)
- **Hardware Connection** → [Hardware Guide](HARDWARE_GUIDE.md)
- **X.on Setup** → [X.on Setup Guide](X_on_Setup_Guide.md)
- **Technical Questions** → [Technical Documentation](DOCUMENTATION.md)

### **Feature Questions**
- **IIT Φ Integration** → [IIT Φ Integration](README_IIT_PHI.md)
- **API Reference** → [Technical Documentation](DOCUMENTATION.md)

---

## 📝 **Contributing to Documentation**

If you find errors or want to improve the documentation:

1. **Report Issues** - Use GitHub Issues for documentation bugs
2. **Suggest Improvements** - Propose enhancements via Pull Requests
3. **Add Examples** - Share your use cases and examples
4. **Update Guides** - Help keep documentation current

---

*Last Updated: January 2025 | Neural Axis BCI v2.0*